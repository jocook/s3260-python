...
if __name__ == "__main__":
    try:
        ...
        print "Deleting VLAN:" #% (template)

        from ucscsdk.mometa.fabric.FabricVlan import FabricVlan

        mo = handle.query_dn("domaingroup-root/fabric/lan/net-192-10.10.192.0_24")
        handle.remove_mo(mo)
        handle.commit()

        mo = handle.query_dn("domaingroup-root/fabric/lan/net-193-10.10.193.0_24")
        handle.remove_mo(mo)
        handle.commit()
